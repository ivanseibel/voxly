...
struct VoxlyConfig: Codable {
    ...
    private static let help: [String: String] = [
        ...
        "llamaContextSize": "Llama context window size in tokens. Prompts and refined output share it; a dictation whose prompts plus a viable output don't fit is inserted without refinement instead of being silently truncated.",
        "llamaGpuLayers": "GPU layers offloaded to Metal ('all' or a number as a string).",
        "refineMaxTokens": "Floor for the refinement output budget, not a ceiling: Voxly scales the budget with the length of the dictation and clamps it to the room left in llamaContextSize.",

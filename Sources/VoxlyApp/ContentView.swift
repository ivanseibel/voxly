...
struct DiagnosisView: View {
    ...
    var body: some View { VStack(alignment: .leading, spacing: 24) { Text("Diagnóstico").font(.title2.weight(.semibold)); Text("Voxly só libera ditado com permissões e modelos locais disponíveis.").foregroundStyle(VoxlyColor.muted)
        ...
        CheckRow(title: "Modelos locais", detail: "Whisper + modelo de refinamento no armazenamento privado", ok: store.status.models, actionLabel: "Abrir pasta", action: { NSWorkspace.shared.selectFile(nil, inFileViewerRootedAtPath: ModelLocator.shared.installFolder) })
